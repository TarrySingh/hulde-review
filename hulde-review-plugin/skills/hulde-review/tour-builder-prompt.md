...
> Used by `/hulde-review` Phase 5. Dispatch as a subagent with this full content as the prompt.
...
```bash
cat > /tmp/hr-tour-input.json << 'ENDJSON'
{
...
```bash
node /tmp/hr-tour-analyze.js /tmp/hr-tour-input.json /tmp/hr-tour-results.json
```
...
After the script completes, read `/tmp/hr-tour-results.json`. Use the structural analysis as your primary guide for designing the tour. Do NOT re-read source files or re-analyze the graph -- trust the script's results entirely.
...
1. Write the JSON array to: `<project-root>/.hulde-review/intermediate/tour.json`
