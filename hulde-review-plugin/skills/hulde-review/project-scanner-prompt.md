...
> Used by `/hulde-review` Phase 1. Dispatch as a subagent with this full content as the prompt.
...
```bash
node /tmp/hr-project-scan.js "<project-root>" "/tmp/hr-scan-results.json"
```
...
After the script completes, read `/tmp/hr-scan-results.json`. Do NOT re-run file discovery commands or re-count lines -- trust the script's results entirely.
...
1. Create the output directory: `mkdir -p <project-root>/.hulde-review/intermediate`
2. Write the JSON to: `<project-root>/.hulde-review/intermediate/scan-result.json`
