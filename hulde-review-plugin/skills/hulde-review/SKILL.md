---
name: hulde-review
description: Analyze a codebase to produce an interactive knowledge graph for code review, architecture understanding, and improvement insights
argument-hint: [options]
---

# /hulde-review

Analyze the current codebase and produce a `knowledge-graph.json` file in `.hulde-review/`. This file powers the interactive dashboard for exploring the project's architecture.
...
3. Create the intermediate output directory:
   ```bash
   mkdir -p $PROJECT_ROOT/.hulde-review/intermediate
   ```
4. Check if `$PROJECT_ROOT/.hulde-review/knowledge-graph.json` exists. If it does, read it.
5. Check if `$PROJECT_ROOT/.hulde-review/meta.json` exists. If it does, read it to get `gitCommitHash`.
6. **Decision logic:**
...
> Write output to: `$PROJECT_ROOT/.hulde-review/intermediate/scan-result.json`

After the subagent completes, read `$PROJECT_ROOT/.hulde-review/intermediate/scan-result.json` to get:
...
> Write output to: `$PROJECT_ROOT/.hulde-review/intermediate/batch-<batchIndex>.json`
>
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.hulde-review/intermediate/layers.json`
> Project: `<projectName>` — `<projectDescription>`
>
> File nodes:
> ```json
...
After the subagent completes, read `$PROJECT_ROOT/.hulde-review/intermediate/layers.json` and normalize it into a final `layers` array. Apply these steps **in order**:
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.hulde-review/intermediate/tour.json`
> Project: `<projectName>` — `<projectDescription>`
> Languages: `<languages>`
>
> Nodes (summarized):
...
After the subagent completes, read `$PROJECT_ROOT/.hulde-review/intermediate/tour.json` and normalize it into a final `tour` array. Apply these steps **in order**:
...
2. Write the assembled graph to `$PROJECT_ROOT/.hulde-review/intermediate/assembled-graph.json`.
...
Pass these parameters in the dispatch prompt:
   ...
   > Validate the knowledge graph at `$PROJECT_ROOT/.hulde-review/intermediate/assembled-graph.json`.
   > Project root: `$PROJECT_ROOT`
   > Read the file and validate it for completeness and correctness.
   > Write output to: `$PROJECT_ROOT/.hulde-review/intermediate/review.json`

4. After the subagent completes, read `$PROJECT_ROOT/.hulde-review/intermediate/review.json`.
...
1. Write the final knowledge graph to `$PROJECT_ROOT/.hulde-review/knowledge-graph.json`.

2. Write metadata to `$PROJECT_ROOT/.hulde-review/meta.json`:
   ```json
   {
...
4. Report a summary to the user containing:
   ...
   - Path to the output file: `$PROJECT_ROOT/.hulde-review/knowledge-graph.json`

5. Only automatically launch the dashboard by invoking the `/hulde-dashboard` skill if final graph validation passed after normalization/review fixes.
