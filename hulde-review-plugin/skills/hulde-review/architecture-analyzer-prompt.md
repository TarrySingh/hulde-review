...
> Used by `/hulde-review` Phase 4. Dispatch as a subagent with this full content as the prompt.
...
```bash
cat > /tmp/hr-arch-input.json << 'ENDJSON'
{
...
```bash
node /tmp/hr-arch-analyze.js /tmp/hr-arch-input.json /tmp/hr-arch-results.json
```
...
After the script completes, read `/tmp/hr-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
1. Write the JSON array to: `<project-root>/.hulde-review/intermediate/layers.json`
