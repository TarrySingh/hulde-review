    ...
    <div className="h-full w-full overflow-auto p-5 animate-fade-slide-in">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
          <div className="text-2xl font-mono font-medium text-hulde-green">{nodes.length}</div>
          <div className="text-[11px] text-text-muted uppercase tracking-wider mt-1">Nodes</div>
        </div>
        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
          <div className="text-2xl font-mono font-medium text-hulde-green">{edges.length}</div>
          <div className="text-[11px] text-text-muted uppercase tracking-wider mt-1">Edges</div>
        </div>
        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
          <div className="text-2xl font-mono font-medium text-hulde-green">{layers.length}</div>
          <div className="text-[11px] text-text-muted uppercase tracking-wider mt-1">Layers</div>
        </div>
        <div className="bg-elevated rounded-lg p-3 border border-border-subtle">
          <div className="text-2xl font-mono font-medium text-hulde-green">{Object.keys(typeCounts).length}</div>
      ...
      {project.languages.length > 0 && (
        <div className="mb-5">
          <h3 className="text-[11px] font-semibold text-hulde-green uppercase tracking-wider mb-2">Languages</h3>
      ...
      {project.frameworks.length > 0 && (
        <div className="mb-5">
          <h3 className="text-[11px] font-semibold text-hulde-green uppercase tracking-wider mb-2">Frameworks</h3>
          <div className="flex flex-wrap gap-1.5">
      ...
      {hasTour && (
        <button
          onClick={startTour}
          className="w-full bg-hulde-green/10 border border-hulde-green/30 text-hulde-green text-sm font-medium py-2.5 px-4 rounded-lg hover:bg-hulde-green/20 transition-all duration-200"
        >
          Start Guided Tour
        </button>
      )}
