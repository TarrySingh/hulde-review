...
const complexityColors: Record<string, string> = {
  ...
  moderate: "text-hulde-green-dark",
  complex: "text-[#c97070]",
};
...
export default function CustomNode({
  ...
  let extraClass = "";
  if (data.isSelected) {
    extraClass = "ring-2 ring-hulde-green node-glow";
  } else if (data.isTourHighlighted) {
    extraClass = "ring-2 ring-hulde-green-dark animate-hulde-pulse";
  } else if (data.isHighlighted) {
    const score = data.searchScore ?? 1;
    if (score <= 0.1) {
      extraClass = "ring-2 ring-hulde-green-bright";
    } else if (score <= 0.3) {
      extraClass = "ring-2 ring-hulde-green";
    } else {
      extraClass = "ring-1 ring-hulde-green-dark/60";
    }
  }
