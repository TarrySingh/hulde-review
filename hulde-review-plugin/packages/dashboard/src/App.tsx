...
import { validateGraph } from "@hulde-review/core/schema";
...
function App() {
  ...
  useEffect(() => {
    fetch("/knowledge-graph.json")
      .then((res) => {
        if (!res.ok) {
          // No graph yet — this is normal before running /hulde-review
          return null;
        }
        return res.json();
      })
      .then((data: unknown) => {
        if (!data) return; // No graph available — show empty state
      ...
      <header className="flex items-center justify-between px-5 py-3 bg-surface border-b border-border-subtle shrink-0">
        <div className="flex items-center gap-5">
          <h1 className="font-serif text-lg text-text-primary tracking-wide">
            {graph?.project.name ?? "Hulde Review"}
          </h1>
